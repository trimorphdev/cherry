...
pub enum Spacing {
    /// Either there is no token after this one, or there is no whitespace between this token and the next
    /// token.
    None,
...
/// A comment token.
/// 
/// This will never be outputted directly by the lexer.  Comments may be found in tokens that have
...
pub struct Comment {
    ...
    /// The value of this comment, without any trailing whitespace or parts of the comment syntax, such
    /// as `//` or `/**/`.
    pub value: String,
...
/// 
/// By this point, the lexer has already converted this token to a usable integer value, rather than
